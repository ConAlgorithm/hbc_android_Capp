...
/**
 * @ClassName: TwoDimensionUtlis
 * @Description: 二维码工具类
 * @Author QiZhang
 * @Date 2018/2/27
 */

public class TwoDimensionUtlis {
    ...
    public static Bitmap createQRCodeBitmap(String content, int size, @Nullable Bitmap logoBitmap, float logoPercent) {
        return createQRCodeBitmap(content, size, "UTF-8", "M", "0", Color.BLACK, Color.WHITE, null, logoBitmap, logoPercent);
    }
    ...
    private static Bitmap addLogo(@Nullable Bitmap srcBitmap, @Nullable Bitmap logoBitmap, float logoPercent) {
        ...
        /** 4. 使用Canvas绘制,北京的白色圆 */
        float r = srcWidth / 2 * logoPercent > srcHeight / 2 * logoPercent ? srcWidth / 2 * logoPercent : srcHeight / 2 * logoPercent;
        Bitmap bitmap_ = Bitmap.createBitmap(srcWidth, srcHeight, Bitmap.Config.ARGB_8888);
        Canvas canvas_ = new Canvas(bitmap_);
        canvas_.drawBitmap(srcBitmap, 0, 0, null);
        final Paint paint = new Paint();
        paint.setStyle(Paint.Style.FILL);
        paint.setAntiAlias(true);
        paint.setColor(Color.WHITE);
        canvas_.drawCircle(srcWidth / 2 , srcHeight / 2, r + 8, paint);
