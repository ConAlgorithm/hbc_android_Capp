...
public class RecyclerViewPager extends RecyclerView {
    ...
    public int getCurrentPosition() {
        int curPosition;
        if (getLayoutManager().canScrollHorizontally()) {
            curPosition = RVViewUtils.getCenterXChildPosition(this);
        } else {
            curPosition = RVViewUtils.getCenterYChildPosition(this);
        }
        if (curPosition < 0) {
    ...
    protected void adjustPositionX(int velocityX) {
        ...
        if (childCount > 0) {
            int curPosition = RVViewUtils.getCenterXChildPosition(this);
            ...
            if (targetPosition == curPosition
                ...
                View centerXChild = RVViewUtils.getCenterXChild(this);
    ...
    protected void adjustPositionY(int velocityY) {
        ...
        int childCount = getChildCount();
        if (childCount > 0) {
            int curPosition = RVViewUtils.getCenterYChildPosition(this);
            ...
            if (targetPosition == curPosition
                    && (!mSinglePageFling || mPositionOnTouchDown == curPosition)) {
                View centerYChild = RVViewUtils.getCenterYChild(this);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN && getLayoutManager() != null) {
            mPositionOnTouchDown = getLayoutManager().canScrollHorizontally()
                    ? RVViewUtils.getCenterXChildPosition(this)
                    : RVViewUtils.getCenterYChildPosition(this);
            if (DEBUG) {
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == SCROLL_STATE_DRAGGING) {
            mNeedAdjust = true;
            mCurView = getLayoutManager().canScrollHorizontally() ? RVViewUtils.getCenterXChild(this) :
                    RVViewUtils.getCenterYChild(this);
            if (mCurView != null) {
            ...
            if (mNeedAdjust) {
                int targetPosition = getLayoutManager().canScrollHorizontally() ? RVViewUtils.getCenterXChildPosition(this) :
                        RVViewUtils.getCenterYChildPosition(this);
                if (mCurView != null) {
